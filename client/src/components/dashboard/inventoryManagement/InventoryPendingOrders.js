    ...
    for (let i = 0; i < products.length; i++) {
      if (products[i]._id === id) {
        ...
        if (response.ok) {
          ...
          const responseForDeleteIPO = await fetch(
            "/api/inventoryProductOrder/" + ipoID,
            {
              method: "DELETE",
            }
          );
  ...
  const updateOrderStatusAll = async (customerOrder) => {
    for (let i = 0; i < customerOrder.length; i++) {
      const response = await fetch(
        "/api/orders/packaged/" + customerOrder[i]._id,
        {
          method: "PATCH",
          body: JSON.stringify({ Status: "Packaged" }),
          headers: {
  ...
  const updateRmStock = async (rmName, rmQty, rmOrderID) => {
    ...
    for (let i = 0; i < rawMaterials.length; i++) {
      if (rawMaterials[i].raw_material_name === rmName) {
        ...
        const response = await fetch(
          "/api/inventoryRawMaterials/" + rawMaterials[i]._id,
          {
            method: "PATCH",
    ...
    <main id="main" className="main">
      ...
      <div className="card">
        <div className="card-body">
          ...
          <div
            className="card-body"
            style={{ height: "352px", overflow: "auto" }}
          >
            <table
            ...
            >
              ...
              <tbody>
                {inventoryProductOrders &&
                  inventoryProductOrders.map((ipo) => (
                    <tr>
                      <th scope="row">
                        {inventoryProductOrders.indexOf(ipo) + 1}
                      </th>
      ...
      <div className="card">
        <div className="card-body">
          <center>
            <h5 className="card-title">Raw Materials</h5>
          </center>
        ...
        <div className="card-body">
                    ...
                    <tr key={custOrder._id}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => updateOrderStatus(custOrder._id)}
                          title="Set as packaged"
                        >
          ...
          <div>
            ...
            <button
              ...
              onClick={() => updateOrderStatusAll(customerOrder)}
              title="Set all as packaged"
            >
              Pack All
            </button>
          </div>
